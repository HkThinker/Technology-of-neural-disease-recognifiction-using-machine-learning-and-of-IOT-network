...
temp=randperm(100953); % 1086*77   756*754

P_train=sp_al(temp(10096:end),1:88)';
T_train=sp_al(temp(10096:end),89)';
M=size(P_train,2);

P_test=sp_al(temp(1:10095),1:88)';
T_test=sp_al(temp(1:10095),89)';
N=size(P_test,2);

% % 
% load("sp_al_del.mat")
% 
% temp=randperm(89391); % 1086*77   756*754
% 
% P_train=sp_al(temp(8940:end),1:88)';
% T_train=sp_al(temp(8940:end),89)';
% M=size(P_train,2);
% 
% P_test=sp_al(temp(1:8939),1:88)';
% T_test=sp_al(temp(1:8939),89)';
% N=size(P_test,2);
...
% 但是应该始终和输入层数据结构保持一致
P_train=double(reshape(P_train,[88,1,1,M]));
P_test=double(reshape(P_test,[88,1,1,N]));
